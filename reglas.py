...
def filtrar_ayudantes_inteligente(hermano_titular, lista_hermanos, aptitud_filtro):
    # Filtro estricto de aptitudes para separar secciones de forma obligatoria
    candidatos = [h for h in lista_hermanos if aptitud_filtro in h.get("aptitudes", [])]
    
    # Si es el casillero de Ayudante en Maestros, aplicamos filtros de genero y consanguinidad
    ...
    # Formateo limpio para el menu desplegable de la web
    lista_listas = []
    for h in candidatos:
...
def generar_pdf_estilo_oficial(lectura_cabecera, fecha_cabecera, materias, asignados):
    ...
    # Tipografías y Estilos de Color Oficiales de Imprenta
    ...
    # --- Cabecera Principal Izquierda ---
    cab_izq = [
        Paragraph(f"{fecha_cabecera}", est_fecha),
        Paragraph(f"{lectura_cabecera}", est_lectura)
    ]
    
    # --- Cabecera Firma Presidente Derecha (Ajuste de Ancho Corregido) ---
    ...
    # --- Fila Canción 40 y Oración Inicial ---
    ...
    t_c1 = Table(datos_cancion_1, colWidths=[200, 170, 170])
    ...
    # --- Fila Conclusión y Canción 60 Final ---
    t_c3 = Table([[Paragraph("Palabras de conclusión (3 mins.)", est_cab_tit), Paragraph("🎵 <b>Canción 60</b> y oración", est_cab_tit), Paragraph("", est_hnos)]], colWidths=[200, 170, 170])
