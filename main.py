...
def generar_pdf_estilo_oficial(mes_activo, semana_act, materias, asignados):
    ...
    secciones_mapeadas = {
        ...
        "Lectura": {"titulo": "TESOROS DE LA BIBLIA", "color": "#3A7885", "estilo_t": est_t_tesoros}
    }
    
    seccion_actual = ""
    barra_tesoros_dibujada = False
    
    # --- 3. BUCLE PRINCIPAL CON BLOQUEADOR DE BARRAS REPETIDAS ---
    for k in sorted(materias.keys(), key=lambda x: int(x) if x.isdigit() else 999):
        m = materias[k]
        sec_materia = m.get("seccion", "Tesoros")
        
        # Si cambia de sección, evaluamos si corresponde pintar una barra nueva
        if sec_materia != seccion_actual:
            ...
            # ELIMINACIÓN DE BARRA DUPLICADA: Si la sección es Tesoros o Lectura, solo permitimos dibujarla UNA VEZ en todo el PDF
            dibujar_barra = True
            if seccion_actual in ["Tesoros", "Lectura"]:
                if barra_tesoros_dibujada:
                    dibujar_barra = False
                else:
                    barra_tesoros_dibujada = True
            
            if dibujar_barra:
                t_tit = Table([[Paragraph(f"<b>{conf['titulo']}</b>", est_letra_blank)]], colWidths=)
        ...
        t_fila = Table([fila_materia], colWidths=)
    ...
    t_c_fin = Table([datos_conclusion], colWidths=)
